...
public class IPSeeker {
	...
	private IPSeeker()  {
		...
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(Utility.IP_FILE, "r");
	...
	public String getCountry(byte[] ip) {
		...
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = Utility.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
	    return getCountry(Utility.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
	    return getArea(Utility.getIpByteArrayFromString(ip));
	}
	...
	private String readString(long offset) {
		try {
			...
			if(i != 0) 
			    return Utility.getString(buf, 0, i, "GBK");
		} catch (IOException e) {			
		    e.printStackTrace();
		}
		return "";
