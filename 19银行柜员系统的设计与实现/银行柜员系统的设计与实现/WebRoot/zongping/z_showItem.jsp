...
%>
<jsp:useBean id="z_showitem" scope="page" class="firm.firm"/>
<%
    String itemID=request.getParameter("itemID");	
    String sql="select * from I_s_item where itemID='"+itemID+"'";
    ResultSet rs=z_showitem.executeQuery(sql);
	//定义数据类型并初始化
    ...
    if(rs.next())
	{
		...
		rs.close();
        z_showitem.closeStmt();
        z_showitem.closeConn();
	}
	...
	ResultSet rs2=z_showitem.executeQuery(sql2);
	String pSql="select * from I_distribute where itemID='"+itemID+"'";
	ResultSet pRs=z_showitem.executeQuery(pSql);
                ...
                <tr> 
                  <td>                    
                      ...
                      <tr> 
                        ...
                        <td bgcolor="#f2f2f2"> <font color="#000099"><%=applicant1%>（<%=grade1%>）（<%=education1%>）</font> 
                        </td>
                        </tr>
                        <tr> 
                    ...
                    <table width="95%" border="0" cellspacing="1" cellpadding="1" align="center">
					  ...
					  { String item_name=pRs.getString("item_name");
						...
						%>
                      <tr> 
                        <td bgcolor="#f2f2f2" colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;<a href="z_showMind.jsp?itemID=<%=itemID%>&identityID=<%=identityID%>" target="_blank"><%=expert[i]%></a><%=expertMind%></td>
                      </tr>
                      <% i++; } %>
                      <tr> 
                        <td colspan="2">&nbsp;</td>
                      ...
                      <tr> 
                        <td bgcolor="#d9d8dc" align="right" width="50%"><a href="z_leadingMind.jsp?itemID=<%=itemID%>"><img src="../images/leadingMind.gif" width="80" height="22" border="0"></a>&nbsp;</td>
                        <td bgcolor="#d9d8dc">&nbsp;<INPUT onclick=javascript:window.close(); type=image src="../images/shutdown.gif" width="80" height="22"></td>
                      </tr>
                    </table>
                  </td>
                </tr>
