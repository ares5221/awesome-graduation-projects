			...
			<script type="text/javascript">
				function checkreader(){
					...
					if(barcodeval==""){//非空校验
						...
						//get请求
						window.location.href="ReaderServlet?operation=findreaderinfo&pagename=bookrenew&readerbarcode="+barcodeval;	
				    }
				}
			   ...
			   function bookRenew(borrowid){
			   		//alert(borrowid);
			   		$.post("ReaderServlet",{operation:"bookrenew",pagename:"bookrenew",borrowid:borrowid},function(result){
			   		     if(result=="true"){
			   		     	alert("续借成功");
			   		     	checkreader();	
			   		     }else if(result=="false"){
			   		     	alert("续借失败");
			   		     }else if(result=="alreadyrenew"){
			   		        alert("已经续借一次，不能再次续借");
			   		     }else{
			   		        alert("已经逾期"+result+"天，缴纳逾期费用后才可续借");
			   		     }
			   		     
			   		});
			   		/* window.location.href="ReaderServlet?operation=bookrenew&pagename=bookrenew&readerid="+rid+"&bookid="+bid;	 */
			   }
		</script>
	</head>
		        ...
		        <td height="72" align="center" valign="top" background="Images/main_booksort_1.gif" bgcolor="#F8BF73"><table width="96%" border="0" cellpadding="1" cellspacing="1" bordercolor="#F8BF73">
		                  ...
		                  <tr>
		                    <td align="center"><table width="96%" border="0" cellpadding="0" cellspacing="0">
		                        <tr>
		                          ...
		                          <td>读者类型：
		                            <input name="readerType" type="text" id="readerType" value="${reader.readerType.name }">
		                           </td>
		                        </tr>
		                        <tr>
		                          <td height="27">证件类型：
		                          ...
		                          <td>可借数量：
		                            <input name="number" type="text" id="number" value="${reader.readerType.number-fn:length(readerBorrowList) }" size="17">册
		                            &nbsp;
		                            </td>
		                        </tr>
		                    </table></td>
		          ...
		          <tr>
		            <td valign="top"><table width="100%" height="35" border="1" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bordercolorlight="#FFFFFF" bordercolordark="#F6B83B" bgcolor="#FFFFFF">
		                ...
		                <c:if test="${fn:length(readerBorrowList)>0 }">
		                	<c:forEach items="${readerBorrowList }" var="each">
				                <tr align="center" bgcolor="#e3F4F7">
					                  ...
					                  <td width="12%" bgcolor="#FFF9D9"><input name="Button22" type="button" class="btn_grey" value="续借" onClick="bookRenew('${each[0] }');"></td>
				                </tr>   
		                    </c:forEach>
		                </c:if>
