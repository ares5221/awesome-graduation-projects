...
<script type="text/javascript"
	...
	<body>
		<form action="servlet/UserServlet?method=UserAdd&i=2"  name="form1" onsubmit="return validator(this)"   method="post"
 onsubmit="return checkForm('form2');">
			<table class=editTable cellSpacing=1 cellPadding=0 width="100%"
				...
				<tr>
					...
					<td colspan="3" bgcolor="#FFFFFF">
						...
						<c:forEach items="${ roleList}" var="role" >
							<c:if test="${role.role_name=='管理员' }">
								<option value="${ role.role_ID}" selected="selected">${ role.role_name}</option>
							</c:if>
							<c:if test="${role.role_name!='管理员' }">
								<option value="${ role.role_ID}" disabled="disabled">${ role.role_name}</option>
							</c:if>	
						</c:forEach>
				...
				<tr>
					...
					<td colspan="3" bgcolor="#FFFFFF">
						<input type="text" style="width: 145px"   maxlength="20"  name="userAddman" valid="required" value="${user.user_addman }" errmsg="添加人不能为空!" >
					</td>
					<td bgcolor="#FFFDF0">
						<div align="center">
							E_mail：
