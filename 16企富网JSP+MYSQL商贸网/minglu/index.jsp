...
<table border=0 width=750 cellspacing=3 bgcolor=#F2F2F2 cellpadding=2 align=center><tr height=15><td align=left>共<font color=ff6600><b><%=(i+20)/20%></b></font>页&nbsp;第<font color=ff6600><b><%=pageInt%></b></font>页&nbsp;共<font color=ff6600><b><%=i%></b></font>条信息</TD><td align=right>转到第<%for(int j=1;j<(i+40)/20;j++){%>&nbsp;<a href="index.jsp?page=<%=j%>"><font color=red><%=j%></font></a><%}%>&nbsp;页</td><td>
<%if (pageInt>1){%><a href=index.jsp?page=<%=pageInt-1%>><%}%>[上一页]</a>
<%int bb=(i+20)/20;%><%if (pageInt<bb){%><a href=index.jsp?page=<%=pageInt+1%>><%}%>[下一页]</a></td></tr></form> 
