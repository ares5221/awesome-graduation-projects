...
<%String txtitle=request.getParameter("txtitle");
   if(txtitle==null)txtitle = "";
	if(txtitle.equals("职位关键字")){
	txtitle ="";
	}%>
...
rs=stmt.executeQuery("SELECT * from job where topic like '%"+txtitle+"%' and lei='"+lei+"' order by id desc");
...
<%
rs=stmt.executeQuery("SELECT count(*) from JOB where topic like '%"+txtitle+"%' and lei='"+lei+"' order by id desc");
while(rs.next()) i = rs.getInt(1) ;
%>

  <td width="350" class="zl">共找到招骋职位<font color="#FF0000"><%=i%></font>个 
                                        分<%=(i+20)/20%>页 当前为第<%=pageInt%>页</td>
                                      <td valign="middle">
<div align="right"> 
                                          <p class="zl">
										  
									转到第&nbsp;<%for(int j=1;j<(i+40)/20;j++){%><a href="search.jsp?page=<%=j%>&txtitle=<%=txtitle%>&lei=<%=lei%>"><font color=red><%=j%></font></a>&nbsp;<%}%>页
